...
- [~] **1.2** Audit all state mutations in `game_state.gd`. **Movement commands done** (register/unregister, move_to_cell, move_to_pos, walk_path, stop, change_speed). **Remaining:** items (spawn/remove/pick_up/drop/transfer/endocytose/exocytose/cancel_endocytosis), physics (register/unregister/throw/apply_area_impulse), pendulums (register/unregister), abilities (queue_ability/cancel_queued_ability), dodge_roll. Lint pass for the audit not yet built.
