...
const MODEL_PATH := "res://resources/models/fauna/sapscrap/sapscrap.gltf"
const BIN_PATH := "res://resources/models/fauna/sapscrap/sapscrap.bin"
const BODY_TEXTURE := "res://resources/models/fauna/sapscrap/sapscrap_shell.png"
const MOUTH_TEXTURE := "res://resources/models/fauna/sapscrap/sapscrap_mouth.png"
const SIGNAL_TEXTURE := "res://resources/models/fauna/sapscrap/sapscrap_claw.png"
const SIGNAL_EMISSIVE := "res://resources/models/fauna/sapscrap/ferrule_signal_emissive.png"
const WRAPPER_PATH := "res://scenes/props/biota/sapscrap_visual.tscn"
const MASTER_PATH := "res://../blender/fauna/sapscrap/sapscrap.blend"
const EXPORT_HELPER_PATH := "res://../blender/fauna/sapscrap/export_sapscrap.py"
const EXPECTED_ANIMATIONS := ["idle", "windup", "lunge", "bite"]
...
func _run() -> void:
	...
	_verify_wrapper()
	print("SAPSCRAP MODEL: %d checks, %d failures" % [_checks, _failures])
	quit(0 if _failures == 0 else 1)


func _verify_portable_files() -> void:
	for path in [MODEL_PATH, BIN_PATH, BODY_TEXTURE, MOUTH_TEXTURE, SIGNAL_TEXTURE]:
		_check(FileAccess.file_exists(path), "%s exists" % path.get_file())
	for path in [BODY_TEXTURE, MOUTH_TEXTURE, SIGNAL_TEXTURE]:
...
func _verify_gltf_sidecars() -> void:
	...
	_check(not buffers.is_empty() and str((buffers[0] as Dictionary).get("uri", "")) == "sapscrap.bin",
	...
	_check(image_uris.has("sapscrap_shell.png")
		and image_uris.has("sapscrap_mouth.png")
		and image_uris.has("sapscrap_claw.png"),
		"glTF binds all three external paint sheets")
...
func _verify_wrapper() -> void:
	...
	_check(FileAccess.file_exists(WRAPPER_PATH)
		and wrapper_text.contains("sapscrap.gltf")
	...
	_check(wrapper_text.contains("../blender/fauna/sapscrap/sapscrap.blend")
		and wrapper_text.contains("../blender/fauna/sapscrap/export_sapscrap.py"),
	...
	_check(str(instance.get_meta("asset_contract", "")) == "editable_3d_v1"
		and str(instance.get_meta("visual_identity", "")) == "sapscrap_maw_v1"
		and str(instance.get_meta("motion_model", "")) == "windup_lunge_bite",
		"wrapper advertises the portable identity and slinky motion contract")
	var roles: Dictionary = instance.get_meta("action_roles", {})
	_check(roles.size() == 4 and roles.has("Sapscrap_Windup")
		and roles.has("Sapscrap_Lunge") and roles.has("Sapscrap_Bite")
	...
	_check(facing_socket != null and impact_socket != null and rear_socket != null
		and facing_socket.position.z > 0.7
		and impact_socket.position.z > 0.6
		and rear_socket.position.z < -0.1,
	...
	_check(albedo_paths.has(BODY_TEXTURE)
		...
		"shell, mouth, and claw material families retain distinct external textures")
	...
	_check(joined_names.contains("mouth")
		...
		and joined_names.contains("claw"),
		"named mouth, segment, anchor, and claw parts survive import")
	...
	_check(bounds.size.x >= 0.8 and bounds.size.x <= 1.6
		and bounds.size.y >= 0.6 and bounds.size.y <= 1.3
		and bounds.size.z >= 0.7 and bounds.size.z <= 1.5,
