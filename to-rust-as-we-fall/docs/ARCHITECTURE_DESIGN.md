...
Full self-contained image prompts (prepend §1 preamble). Each produces a "hero" reference for one buildable type. Template per entry: **Read** (silhouette priority), **Massing/knobs**, **Two-read**, **Prompt**. §4.1–4.16 are the generic + district-anchor set, §4.17– the named set-pieces, and **§4.18–4.24 the intermediate connective fabric** (the commerce / manufacturing / supply-chain / office economy that fills *between* district cores).
...
### Intermediate fabric — the connective economy between district cores (§4.18–4.24)

Generic archetypes like §4.1–4.16, but authored for the **low-intensity fill between the district hero cores** — the working economy that makes the named districts feel embedded rather than staged. Where §4.7–4.15 are the landmark a stretch is *about*, these are what the player walks *past* to reach it: commerce, informal trade, manufacturing, storage, logistics interchange, clerical offices, and the undifferentiated mixed-use infill that packs every remaining gap. They read lower, denser, and more repetitive than the cores they sit between, and most carry a little of each neighbouring district's idiom bleeding in from both sides. Canon anchors: the supply chain for inelastic goods concentrating near the central facility (GDD §4.4, "proximity to the central facility tracks with access to institutional resources"); automation displacing the productive purpose (GDD §8, the same End-of-History logic the simulation runs on the wellness side); the informal/gray-market worker economy running in parallel to credits (GDD §5.8); the Aghora as a counterfeit agora built on stolen iron and glucose (GDD §5.9.3); and manufactured desire / the society of the spectacle (GDD §18.3, Debord + Simmel's blasé attitude). Each prompt prepends the §1 preamble and closes with the standard composition line.

### 4.18 Retail arcade run — Zone 2 commercial fabric (the everyday street)
**Read:** the §4.6 convenience outlet multiplied into a street-length terrace — four to six identical small shopfronts under one continuous sagging canopy, every frontage clad in overlapping aspirational advertising, ALWAYS OPEN glowing over half-shuttered failure. The ordinary commercial run the generator lays between destinations.
**Massing/knobs:** `shell=ring_corridor_segment (shallow curved retail terrace)`, `storeys=2`, `ground_floor_tall=true`, `rhythm=even_bay (identical bays, alveolar size-jitter)`, `window=shuttered_metal (ground, half-rolled) + capillary_slit (upper)`, `projection=slat_canopy (continuous, sagging) + hoarding_band 🔧 (ad cladding over the upper storey)`, `door=cycling_slab`, `furniture=[toll_meter (Flow Optimization), drink machine]`, `sign=corporate_rebrand × always_open_convenience, layered/overlapping`, `decay=ferric_bleed + oxide_dust (medium)`, `emissive=terminal_green ad-glow + always-open signs`.
*(Placement scope: **capitalist-idiom** — weight to The Hypelines and The Cleanstreets Initiative per GDD §4.11; do NOT place in the socialist collectives (Greenfields, The Honeycomb), whose idiom has no always-open commerce.)*
**Two-read:** a retail strip AND an absorptive brush-border — a lumen wall lined with identical microvilli-frontages, every one straining to pull value from whatever passes down the street.
**Prompt:** *A complete freestanding retail arcade run of a Zone 2 commercial street: a shallow curved terrace of five identical small shopfronts under one continuous faceted slat-canopy that sags between its posts — PS2-era low-poly, nothing gridded, the bays alveolar (the same frontage shape at subtly varying widths). Each tall ground floor is a half-rolled rusted metal shutter over a dark counter; narrow capillary-slit windows run above, and over them the whole upper storey is clad in overlapping aspirational advertising hoardings, glossy panels layered one over the next. ALWAYS OPEN signs glow pristine terminal-green (#5ce87f) along the run while ferric-red streaks bleed down the muted-teal walls behind the ads and rust dust settles on every ledge. A single-file "Flow Optimization" credit meter pinches the arcade entry; a worn detour path skirts it. Warm cream trim, pixel-art tiled textures at 32px/m. The street whose lights never go out, selling to no one. Frame the complete freestanding building — full silhouette from ground line to crown, three-quarter street-level view at a slight low angle, near-black void beyond.*

### 4.19 Gray-market stall cluster — the informal economy (Aghora edge)
**Read:** the unsanctioned counter-market that grows in the cracks between formal blocks — a dense clot of improvised stalls and barter tables under patched awnings, wired into stolen power. Where the collective only allocates, here comparison and wanting are finally permitted. No toll gate; a lookout perch instead.
**Massing/knobs:** `shell=fractal_branch_cluster (accreted stall pods, no master plan)`, `storeys=1 (+ lofted sleeping shelves)`, `rhythm=alveolar_vary (irregular, jury-rigged)`, `window=improvised / none`, `projection=patched_fabric_canopy 🔧 (layered scavenged sheets)`, `door=none (open stalls)`, `furniture=[barter tables, stolen-power tap 🔧, scavenged crates, a flora screen for cover]`, `sign=hand_lettered 🔧 (no institutional register) over pre-rebrand ghost lettering`, `decay=ferric_bleed + candid_mat + jury_rig (medium)`, `emissive=firelight_orange (open flame, unsanctioned) + one stolen terminal-green tap`.
*(Placement scope: **informal / Aghora-adjacent** — weight to worker outer regions and the cracks between formal blocks; NOT near the central facility, whose supply-chain proximity is the thing this economy routes around.)*
**Two-read:** a black market AND ectopic tissue — an unsanctioned growth metabolizing hoarded iron and glucose outside the regulated distribution, ambiguous between a desperate repair and a cancerous defection.
**Prompt:** *A complete freestanding gray-market stall cluster in the cracks between formal blocks: a dense accreted clot of improvised trading stalls — faceted crate-pods and barter tables budding off one another with no master plan, PS2-era low-poly — sheltered under layered patched fabric awnings scavenged from other buildings. Lofted sleeping shelves are wedged above the stalls. A stolen power tap splices into a conduit overhead, feeding a single terminal-green (#5ce87f) readout amid warm-orange open cooking flames — the only two light anchors, both unsanctioned. Hand-lettered price boards hang where institutional signage would be; older painted cooperative lettering ghosts through beneath them. Muted teal salvage streaked ferric-red, a white fungal candid-mat creeping up the shaded flank, rust dust everywhere, pixel-art tiles. No toll gate — just a raised lookout perch at the mouth. Reads as a black market and as an ectopic growth feeding on hoarded iron outside the regulated flow. Frame the complete freestanding structure — full silhouette from ground line to crown, three-quarter street-level view at a slight low angle, near-black void beyond.*

### 4.20 Automated fabrication shed — manufacturing (the productive purpose, automated away)
**Read:** a long low production hall, most of its automated line dark, a single run still cycling for nobody while finished product piles up unshipped — the automation that displaced the work, still extruding after the demand it fed went quiet.
**Massing/knobs:** `shell=lofted_barrel_shed 🔧 (long low vaulted hall)`, `storeys=1 (tall)`, `rhythm=even_bay (structural ribs)`, `window=capillary_slit clerestory (high band) + scan_grille`, `door=service_hatch (roll-up loading door)`, `crown=sawtooth_clerestory 🔧 + vent stacks`, `projection=conveyor_spur (feeding §4.10 / §4.22)`, `furniture=[automated line segments, robotic actuators frozen mid-motion, unshipped product totes stacked at the dock, dummy relief_console]`, `sign=institutional_project × corporate_rebrand`, `decay=oxide_dust (heavy on the idle machinery) + ferric_bleed`, `emissive=terminal_green on the one live line (telegraph_pulse, cycle) + firelight_orange weld-arc on a cadence`.
*(Placement scope: **supply-chain proximate** — weight to The Hypelines and the zones near the central facility, per GDD §4.4.)*
**Two-read:** a factory AND a secretory acinus — a gland still extruding product on a loop long after the body it fed stopped calling for it.
**Prompt:** *A complete freestanding automated fabrication shed: a long low faceted production hall with a gently vaulted roof and a sawtooth clerestory of narrow capillary-slit windows along its spine — PS2-era low-poly, the ribs even but the mass softly tapered, nothing gridded. A big roll-up loading door stands open at one end where unshipped product totes have stacked up unclaimed. Inside, a run of automated line machinery is visible: most of it dark under a heavy layer of rust-colored oxide dust, robotic actuators frozen mid-motion — but a single line still cycles, its status strip glowing terminal-green (#5ce87f) and pulsing one beat before each cycle, a warm-orange weld-arc flaring on a slow cadence over it. Vent stacks on the roofline; ferric-red bleed down every seam; muted teal metal, warm cream trim, pixel-art tiles. An institutional-project plaque over a corporate rebrand marks the entry. Reads as a factory and as a gland extruding product no one needs. Frame the complete freestanding building — full silhouette from ground line to crown, three-quarter street-level view at a slight low angle, near-black void beyond.*

### 4.21 Bonded warehouse — iron-store depot (supply chain: storage)
**Read:** a near-windowless racked storage block, palletized iron stock locked behind a bonded scan-cage — the depot of inelastic goods that concentrates near the central facility. Its fullness reads as hoarding, not plenty; the stock itself is rusting where it sits.
**Massing/knobs:** `shell=drawer_stack_monolith (horizontal-banded racking variant)`, `storeys=4–5 (racking tiers)`, `rhythm=stacked_drawer (pallet racking)`, `window=pore_round vents (sparse) / none`, `door=blast_bulkhead (bonded) + scan_arch`, `projection=conveyor_spur (intake from §4.10)`, `furniture=[palletized iron stock, loader actuator, bonded scan-cage, toll_meter (class-coded access)]`, `sign=government_aspirational (bonded/rationed designator) + class-coded access plate`, `decay=ferric_bleed (the stock itself oxidizes — iron everywhere) + oxide_dust`, `emissive=terminal_green inventory readouts + cyan-white scan-cage`.
*(Placement scope: **supply-chain proximate** — weight to the central-facility approach and The Hypelines; the fuller and more locked-down, the closer to the centre.)*
**Two-read:** a bonded warehouse AND a ferritin store — iron sequestered in a storage granule behind a membrane, hoarded while the tissue downstream starves for it.
**Prompt:** *A complete freestanding bonded warehouse: a monolithic near-windowless storage block banded floor-to-ceiling with horizontal pallet-racking tiers, gently tapered and chamfered rather than cubic — PS2-era low-poly, monumental but never perfectly gridded. The racks are stacked with palletized iron stock, and the iron is itself rusting in place, so ferric-red bleeds not just down the seams but off the stored goods, staining every tier. Sparse round pore-vents dot the upper wall. The single entry is a bonded scan-cage — a heavy sealed blast-bulkhead behind a cold cyan-white scan-arch and a class-coded credit meter, an elevated conveyor-spur feeding stock in over the top. Terminal-green (#5ce87f) inventory readouts glow at the cage. A government-aspirational bonded-store designator hangs pristine over the rust. Muted teal metal drowning in iron-oxide, rust dust on every rack, pixel-art tiles. Reads as a bonded warehouse and as an iron store sequestered behind a membrane while everything downstream goes without. Frame the complete freestanding building — full silhouette from ground line to crown, three-quarter street-level view at a slight low angle, near-black void beyond.*

### 4.22 Cross-dock transfer yard — supply chain interchange (the logistics node)
**Read:** the ground-level yard where The Hypelines' elevated conveyor-conduits touch down and hand off to ground transport — a covered cross-dock where everything is in transit and nothing at rest. If §4.10's exchange is the heart, this is the capillary bed where the actual exchange happens.
**Massing/knobs:** `shell=fractal_branch_cluster (open covered yard — conveyor descents + dock aprons, no enclosing shell)`, `storeys=1 (canopy over apron)`, `rhythm=even_bay (dock bays)`, `crown=branched_canopy (conveyor descents fanning down)`, `door=roll_up_dock_bays 🔧`, `projection=conveyor_spur (multiple, converging)`, `furniture=[conveyor descents, transfer cradles mid-handoff, rotary diverter valves at the branch nodes, toll_meter, queue rails]`, `sign=corporate_rebrand (Hypelines node-tag) + Flow Optimization`, `decay=ferric_bleed + oxide_dust (heaviest at the transfer joints)`, `emissive=terminal_green diverter meters (telegraph_pulse — surge before each handoff)`.
*(Placement scope: **The Hypelines** — the connective node between §4.10 exchange and §4.21 warehouse; place where conveyor lines converge.)*
**Two-read:** a cross-dock yard AND a synaptic cleft — the narrow gap where cargo is handed vesicle-to-vesicle across the junction, held open only for the transfer window.
**Prompt:** *A complete freestanding cross-dock transfer yard of The Hypelines: an open covered apron where several elevated powered conveyor-conduits fan down from different heights and touch off onto ground-level dock bays — faceted PS2-era low-poly, the descents branching fractally like vessels converging on an exchange bed, nothing gridded. Transfer cradles sit under the descents caught mid-handoff, glinting iron stock crossing from belt to bay. Rotary diverter valves stud the branch nodes; queue rails funnel to a single "Flow Optimization" credit meter glowing terminal-green (#5ce87f), pulsing one beat before each surge passes. A low branched canopy shelters the apron. Ferric-red oxidation is heaviest at the transfer joints, rust dust drifting off the moving stock; muted teal metal, pixel-art tiles. A glossy corporate node-tag reads as a Hypelines address. Reads as a cross-dock yard and as a synaptic cleft where signal is handed across, held open only for the transfer window. Frame the complete freestanding structure — full silhouette from ground line to canopy, three-quarter street-level view at a slight low angle, near-black void beyond.*

### 4.23 Clerical office block — the administrative filler (offices)
**Read:** a mid-rise open-plan office slab, floor after identical floor of cubicle sameness lit cold blue-white, a wellness-feed ribbon crawling across the frontage — the clerical class processing curated information. Peris's office (GDD §9.3) multiplied into a whole building.
**Massing/knobs:** `shell=curved_balcony_tower (office slab, gently lofted)`, `storeys=5–7`, `rhythm=even_bay (identical cubicle bays, alveolar size-jitter per floor)`, `window=ribbon_glazing 🔧 (continuous horizontal band, cold-lit) + capillary_slit at the ends`, `door=enforcement_vestibule (lobby turnstile)`, `crown=service_bulkhead + wellness-feed ribbon-board`, `furniture=[cubicle grid visible through the glazing, relief_console (wellness-terminal variant)]`, `sign=government_aspirational (department designator) + a crawling wellness-feed ticker`, `decay=oxide_dust + sepia_haze (tended and gloomy — no corrosion)`, `emissive=cyan-white cubicle glow (mass) + terminal_green wellness ribbon`.
*(Placement scope: **institutional / central-facility zones** — where the simulation devices and governmental functions concentrate, per GDD §4.4; the decay register is gloom and sameness, not rust.)*
**Two-read:** an open-plan office AND a ganglion — a cluster of identical cell-bodies firing rote signal-traffic, the blasé nucleus that has overloaded into indifference and stopped reacting to what passes through it.
**Prompt:** *A complete freestanding clerical office block: a mid-rise slab of five or six faceted storeys, gently lofted and chamfered rather than cubic — PS2-era low-poly, nothing gridded — its frontage a stack of continuous horizontal ribbon-windows. Through the glazing, floor after identical floor of open-plan cubicle grids is visible, every floor the same layout at a subtly different bay size (optimization, not variety), all of it lit the same cold cyan-white so the whole slab glows like a bank of screens. A wellness-feed ribbon-board crawls terminal-green (#5ce87f) across the crown; the lobby is an enforcement-vestibule turnstile. No rust — the decay here is gloom and sameness, rust-colored dust settling only at the unwatched edges under a faint sepia haze. Muted deep teal panels, warm cream trim, a pristine government-aspirational department designator, pixel-art tiles. Reads as an open-plan office and as a ganglion of identical cell-bodies processing rote traffic, gone indifferent to all of it. Frame the complete freestanding building — full silhouette from ground line to crown, three-quarter street-level view at a slight low angle, near-black void beyond.*

### 4.24 Mixed-use infill block — the connective tissue itself (the intermediate area)
**Read:** the literal in-between building — retail hatches at street level, clerical offices stacked above, a residential half-floor crammed under the crown. The undistinguished mixed-use fabric that packs every gap between the district hero cores; lower, denser and more repetitive than anything it sits between, carrying a little of each neighbour's idiom bleeding in from both sides.
**Massing/knobs:** `shell=fractal_branch_cluster (stacked mixed program, budded annexes)`, `storeys=4 (retail / office / office / crammed residential)`, `ground_floor_tall=true`, `rhythm=even_bay (mixed per storey)`, `window=shuttered_metal (retail) + ribbon_glazing 🔧 (office) + pore_round (crammed residential top)`, `door=cycling_slab + toll_meter`, `projection=slat_canopy (retail) + cantilever_balcony (residential)`, `sign=layered registers (the two neighbouring districts' idioms overlapping on one wall)`, `decay=ferric_bleed + oxide_dust (medium — honest, even wear)`, `emissive=terminal_green retail + cyan-white office glow + one warm-lit residential window`.
*(Placement scope: **everywhere between cores** — the generator's connective-tissue default; blend the two adjacent district presets on the two faces so the block reads as the seam between them. This is the archetype that literally answers "what fills the gap.")*
**Two-read:** mixed-use infill AND the connective stroma itself — the undifferentiated interstitial fabric that fills the space between the organs, carrying the vessels and nerves that run from one to the next.
**Prompt:** *A complete freestanding mixed-use infill block: a squat four-storey mass with smaller annexes budded off it, faceted PS2-era low-poly, denser and plainer than anything around it — nothing gridded. Its program stacks visibly: a tall ground floor of half-rolled retail shutters, two middle floors of continuous cold-lit office ribbon-windows, and a crammed residential half-floor under the crown with round pore-windows and a couple of cantilevered balconies. A sagging slat-awning shades the shops; a credit meter sits at the door. The signage layers two registers at once — a corporate rebrand and a picturesque-community plaque overlapping on the same wall, as if two neighbouring districts' idioms bled together here. Emissive tells the stack apart: terminal-green (#5ce87f) at the retail level, cold cyan-white office glow above, and one lone warm-lit residential window at the top. Muted teal panels with even ferric-red bleed and rust dust, honest wear rather than collapse, warm cream trim, pixel-art tiles. Reads as ordinary mixed-use infill and as the connective stroma between the organs, carrying the vessels that run from one district to the next. Frame the complete freestanding building — full silhouette from ground line to crown, three-quarter street-level view at a slight low angle, near-black void beyond.*
