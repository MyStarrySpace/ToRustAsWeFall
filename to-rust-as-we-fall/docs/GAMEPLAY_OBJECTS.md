...
| **Flure** | `Flure` — `objects/flure.gd` (extends `Interactable`) | The lure transaction: activation nonce, target pulling via enemy `lured` state, settle/park, deferred targets, state authority | `configure(gs, pos, target_ids, attract, radius, color)`, property tuning, `set_enemy_resolver`, connect `flure_activated`; only Sapscraps/Ferrules/Hidras answer (ecology law) |
