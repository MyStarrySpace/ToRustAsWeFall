...
func add_portrait(id: String, display_name: String, color: Color) -> void:
	...
	for stat_def in [["hp", "HP", Color(0.7, 0.3, 0.25)], ["atp", "ATP", Color(0.3, 0.6, 0.35)], ["sta", "STA", Color(0.3, 0.5, 0.7)]]:
		...
		if stat_def[0] == "atp":
			var pip_info := _make_pip_strip(int(GameState.ATP_MAX_PIPS), Vector2(4, 3), stat_def[2], Color(0.05, 0.05, 0.06))
			...
			_set_pip_strip_value(pip_info, GameState.ATP_MAX_PIPS)
			continue
...
func _run_button_label() -> String:
	var mode := "RUN" if _run_active else "WALK"
	if _run_keybind == "":
		return mode
	return "%s  %s" % [mode, _run_keybind]
