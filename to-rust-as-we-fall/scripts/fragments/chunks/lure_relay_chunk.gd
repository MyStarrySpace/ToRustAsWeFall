...
# on a node makes every walk-past route a path vertex straight through it — the lure reads as a movement
# waypoint. Set against the hall's +z wall (well inside HALL_HALF_Z); guards/tend still snap to the
# nearest centerline node, so a walk-past path never pivots on the decoration.
